 ...
 */
public class JetBoyThread extends Thread implements JetPlayer.OnJetEventListener {
    private static final String TAG = "JetBoyThread";

    public interface ViewCallback {
        void setTitleBG(Bitmap titleBG);

        Bitmap getTitleBG();

        void setCurrentBed(int currentBed);

        Bitmap getTitleBG2();

        void setTitleBG2(Bitmap titleBG2);
    ...
    static class Temp implements ViewCallback {
        int mHitStreak = 0;
        int mHitTotal = 0;
        int mCurrentBed = 0;
     ...
     * @param handler
     */
    public JetBoyThread(SurfaceHolder surfaceHolder, Context context, Handler handler, ViewCallback callback) {
        ...
        mCallback.setTitleBG(BitmapFactory.decodeResource(mRes, R.drawable.title_hori));
    ...
    private void doDraw(Canvas canvas) {
        ...
        if (mState == STATE_RUNNING) {
            ...
            if (mCallback.getTitleBG2() == null) {
                mCallback.setTitleBG2(BitmapFactory.decodeResource(mRes, R.drawable.title_bg_hori));
            }
            doDrawPlay(canvas);
    ...
    private void doDrawReady(Canvas canvas) {
        canvas.drawBitmap(mCallback.getTitleBG(), 0, 0, null);
    }

    private void doDrawPlay(Canvas canvas) {
        canvas.drawBitmap(mCallback.getTitleBG2(), 0, 0, null);
    }
        ...
        for (int i = (mDangerWillRobinson.size() - 1); i >= 0; i--) {
            ...
            if (asteroid.mDrawX <= mAsteroidMoveLimitX + 20 && !asteroid.mMissed) {
                ...
                if (mLaserOn) {
                    ...
                    mCallback.setHitStreak(mCallback.getHitStreak() - 1);

                    if (mCallback.getHitStreak() < 0)
                        mCallback.setHitStreak(0);
    ...
    protected void processJetEvent(JetPlayer player, short segment, byte track, byte channel,
        ...
        if (mBeatCount == 1) {
            ...
            if (mCallback.getHitStreak() > 28) {

                // did the bed change?
                if (mCallback.getCurrentBed() != 7) {
                    // did it go up?
                    if (mCallback.getCurrentBed() < 7) {
                        mJet.triggerClip(7);
                    }

                    mCallback.setCurrentBed(7);
            ...
            } else if (mCallback.getHitStreak() > 24) {
                if (mCallback.getCurrentBed() != 6) {
                    if (mCallback.getCurrentBed() < 6) {
                    ...
                    mCallback.setCurrentBed(6);
                    mJet.setMuteArray(muteMask[6], false);
                }
            } else if (mCallback.getHitStreak() > 20) {
                if (mCallback.getCurrentBed() != 5) {
                    if (mCallback.getCurrentBed() < 5) {
                        mJet.triggerClip(5);
                    }

                    mCallback.setCurrentBed(5);
                    mJet.setMuteArray(muteMask[5], false);
                }
            } else if (mCallback.getHitStreak() > 16) {
                if (mCallback.getCurrentBed() != 4) {

                    if (mCallback.getCurrentBed() < 4) {
                        mJet.triggerClip(4);
                    }
                    mCallback.setCurrentBed(4);
                    mJet.setMuteArray(muteMask[4], false);
                }
            } else if (mCallback.getHitStreak() > 12) {
                if (mCallback.getCurrentBed() != 3) {
                    if (mCallback.getCurrentBed() < 3) {
                        mJet.triggerClip(3);
                    }
                    mCallback.setCurrentBed(3);
                    mJet.setMuteArray(muteMask[3], false);
                }
            } else if (mCallback.getHitStreak() > 8) {
                if (mCallback.getCurrentBed() != 2) {
                    if (mCallback.getCurrentBed() < 2) {
                        mJet.triggerClip(2);
                    }

                    mCallback.setCurrentBed(2);
                    mJet.setMuteArray(muteMask[2], false);
                }
            } else if (mCallback.getHitStreak() > 4) {
                if (mCallback.getCurrentBed() != 1) {
